...
if(isset($_GET["product_id"]))
{
  ...
  while($row=mysqli_fetch_assoc($result_query)){
    ...
    $pro_quantidade=$row['PRO_QUANTIDADE'];
    ...
    if($pro_quantidade == 0){
      $pro_quantidade = 'Produto Indisponível';
      $pro_nome = $pro_nome.' - Indisponível';
    }
  }

  $review_query = "SELECT count(*) as FED_NOTA, SUM(FED_NOTA)/count(*) as FED_NOTA FROM tb_feedback WHERE ID_PRODUTO = $id";
  $review_result = mysqli_query($conexao,$review_query);

  while($row=mysqli_fetch_assoc($review_result)){
  $med_nota=$row['FED_NOTA'];
  }
  $med_nota = number_format($med_nota, 2, '.', '');
  $med_nota_inteiro = (int) $med_nota;
?>
<!DOCTYPE html>
<html>
    <head>
    ...
    <body>
        ...
        <section class='product'>
            <div class='product-container'>
                <div class='first-img'>
                    <img src='http://localhost/desenvolvimento/Cadastro-Produto/imagens/<?php echo "$pro_imagem1"; ?>'>
                </div>
                <div class='second-img-group'>
                    <div class='second-img'>
                        <img src='http://localhost/desenvolvimento/Cadastro-Produto/imagens/<?php echo "$pro_imagem1"; ?>' onclick='showImg(this.src)'>
                    </div>
                    <div class='second-img'>
                        <img src='http://localhost/desenvolvimento/Cadastro-Produto/imagens/<?php echo "$pro_imagem2"; ?>' onclick='showImg(this.src)'>
                    </div>
                    <div class='second-img'>
                        <img src='http://localhost/desenvolvimento/Cadastro-Produto/imagens/<?php echo "$pro_imagem3"; ?>' onclick='showImg(this.src)'>
                    </div>
                    <div class='second-img'>
                        <img src='http://localhost/desenvolvimento/Cadastro-Produto/imagens/<?php echo "$pro_imagem4"; ?>' onclick='showImg(this.src)'>
                    </div>
                </div>
            </div>
            <div class='product-datails'>
                <p><a href='http://localhost/desenvolvimento/Inicio/code/index.php' class='back-home'>Home</a> / Produtos</p>
                <h2><?php echo "$pro_nome"; ?></h2>
                <div class='product-rating'>
                  <?php
                  if($med_nota_inteiro==0){
                    echo"
                ...
                <label>Produto: <?php echo "$pro_categoria"; ?></label>
                <br>
                <label>Tamanho: <?php echo "$pro_tamanho"; ?></label>
                <br>
                <label>Cor: <?php echo "$pro_cor"; ?></label>
                <br>
                <label>Material: <?php echo "$pro_material"; ?></label>
                <br>
                <label>Quantidade em Estoque: <?php echo "$pro_quantidade"; ?></label>
                <br><br>
                <a href='#' class='product-submit' data-id=<?php echo "$produto_id"; ?> data-uid=
                <?php 
                ...
                ?>>
                <i class='bx bx-cart'></i> Adicionar no Carrinho</a>
                <a href='feedback.php?product_id=<?php echo "$produto_id"; ?>' class='feedback-submit'><i class='bx bxs-comment-detail'></i> Avalie o Produto</a>
                <h3>Detalhes do Produto</h3>
                <p><?php echo "$pro_descricao"; ?></p>
            </div>
        </section>
        <!-- Related Product -->
        <section class="related-product">
          ...
          <div class="related">
            ...
            while($row=mysqli_fetch_assoc($result_query)){
                ...
                $valor = preg_replace('/[^0-9]/', '', $rel_preco);    
                $valor = bcdiv($valor, 100, 2);
                $valor = strtr($valor, '.', ',');
                echo "
        ...
        <section class="rating">
          ...
          $select_query = "SELECT * FROM tb_feedback WHERE ID_PRODUTO = $id ORDER BY RAND() LIMIT 0,4";
          $result_query=mysqli_query($conexao,$select_query);
